...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self) -> List[float]:
        ...
        if self.last_epoch == 0:
            ...
            return [group["lr"] + (base_lr - self.eta_min) * (1 - math.cos(math.pi / (self.max_epochs - self.warmup_epochs))) / 2
                    for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)]
    ...
    def _get_closed_form_lr(self) -> List[float]:
        ...
        return [self.eta_min + 0.5 * (base_lr - self.eta_min) *
                (1 + math.cos(math.pi * (self.last_epoch - self.warmup_epochs) / (self.max_epochs - self.warmup_epochs))) for base_lr in self.base_lrs]
